...
$time=date("U")."000";
$_SESSION['rx2'][] = "[$time, $round_rx]";
$_SESSION['tx2'][] = "[$time, $round_tx]";
$data['label'] = $int;
$data['data'] = $_SESSION['rx2'];
# to make sure that the graph shows only the
# last minute (saves some bandwitch to)
if (count($_SESSION['rx2'])>60)
{
    $x = min(array_keys($_SESSION['rx2']));
    unset($_SESSION['rx2'][$x]);
}
...
echo '
    {"label":"'.$int.'","data":['.implode($_SESSION['rx2'], ",").']}
    ';